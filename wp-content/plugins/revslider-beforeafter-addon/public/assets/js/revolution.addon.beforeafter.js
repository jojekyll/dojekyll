...
;(function() {
	...
	window.RevSliderBeforeAfter = function(_$, slider, options) {
		...
		$ = _$;
		$.event.special.rsBeforeAfterDestroyed = {remove: function(evt) {evt.handler();}};
	...
	RsAddonBeforeAfter.prototype = {
		...
		onResize: function(e, fromReset) {
			...
			for(var i = 0; i < leg; i++) if(width < this.levels[i]) level = i;
	...
	RsBeforeAfterSlide.prototype = {
		...
		fade: function() {
			...
			if(this.revealLine) punchgs.TweenLite.to(this.revealLine, 0.3, {autoAlpha: 0, ease: punchgs.Power2.easeInOut});
		...
		reveal: function() {
			...
			punchgs.TweenLite.to(this.revealBtn, this.timing, {delay: this.delay, left: btnX, top: btnY, ease: this.animation, onComplete: this.complete});
